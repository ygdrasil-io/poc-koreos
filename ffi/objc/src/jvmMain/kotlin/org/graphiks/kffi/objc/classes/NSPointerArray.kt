...
open class NSPointerArray(val ptr: MemorySegment) {
    ...
    fun initWithPointerFunctions(functions: MemorySegment): MemorySegment {
    ...
    fun pointerAtIndex(index: NSUInteger): MemorySegment {
        val sel = ObjCRuntime.sel("pointerAtIndex:")
        return ObjCRuntime.msgSend(ValueLayout.ADDRESS, ptr, sel, index) as MemorySegment
    }
    
    fun addPointer(pointer: MemorySegment): Unit {
        val sel = ObjCRuntime.sel("addPointer:")
        ObjCRuntime.msgSend(null, ptr, sel, pointer)
    }
    
    fun removePointerAtIndex(index: NSUInteger): Unit {
        val sel = ObjCRuntime.sel("removePointerAtIndex:")
        ObjCRuntime.msgSend(null, ptr, sel, index)
    }
    
    fun insertPointer_atIndex(item: MemorySegment, index: NSUInteger): Unit {
        val sel = ObjCRuntime.sel("insertPointer:atIndex:")
        ObjCRuntime.msgSend(null, ptr, sel, item, index)
    }
    
    fun replacePointerAtIndex_withPointer(index: NSUInteger, item: MemorySegment): Unit {
