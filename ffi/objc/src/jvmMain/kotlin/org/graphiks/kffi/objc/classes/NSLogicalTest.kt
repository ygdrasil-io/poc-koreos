 ...
 */
open class NSLogicalTest(ptr: MemorySegment) : NSScriptWhoseTest(ptr) {
    companion object {
        private val _class: MemorySegment by lazy { ObjCRuntime.getClass("NSLogicalTest") }
        
    }
    
    fun initAndTestWithTests(subTests: MemorySegment): MemorySegment {
    ...
    fun initOrTestWithTests(subTests: MemorySegment): MemorySegment {
        val sel = ObjCRuntime.sel("initOrTestWithTests:")
        return ObjCRuntime.msgSend(ValueLayout.ADDRESS, ptr, sel, subTests) as MemorySegment
    }
    
    fun initNotTestWithTest(subTest: MemorySegment): MemorySegment {
