...
class Win32WglContext(hwndAddr: Long) : GlContext {
    ...
    private fun create() {
        ...
        pfd.set(ValueLayout.JAVA_SHORT, 0L, 40.toShort())
        pfd.set(ValueLayout.JAVA_SHORT, 2L, 1.toShort())
        pfd.set(JAVA_INT, 4L, 0x4 or 0x20 or 0x1)
        pfd.set(ValueLayout.JAVA_BYTE, 8L, 0.toByte())
        pfd.set(ValueLayout.JAVA_BYTE, 9L, 32.toByte())
        pfd.set(ValueLayout.JAVA_BYTE, 16L, 8.toByte())
        pfd.set(ValueLayout.JAVA_BYTE, 23L, 24.toByte())
        pfd.set(ValueLayout.JAVA_BYTE, 24L, 8.toByte())
        pfd.set(ValueLayout.JAVA_BYTE, 26L, 0.toByte())
