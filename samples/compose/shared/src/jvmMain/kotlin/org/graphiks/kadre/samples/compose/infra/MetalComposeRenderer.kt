/**
 * macOS present path: renders the shared [ComposeSceneHost] into a Kadre CAMetalLayer via
 * Skiko's Metal backend.
 *
 * Per frame: nextDrawable (CAMetalLayer) → drawable.texture → Skia BackendRenderTarget(Metal)
 * → Surface → host.pumpAndRender(canvas) → flush → [commandBuffer presentDrawable].
 *
 * All Metal/ObjC calls go through Kadre's public [ObjCRuntime] FFM bridge; the only raw FFM
 * downcall added here is the C function `MTLCreateSystemDefaultDevice()`.
 */
...
class MetalComposeRenderer(
    ...
    init {
        // MTLDevice — MTLCreateSystemDefaultDevice() is a C function, not an ObjC message.
        device = createSystemDefaultDevice()

        // Configure the CAMetalLayer (Kadre creates it device-less).
