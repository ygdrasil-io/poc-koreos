...
| `Window::drag_window()` | `Window.dragWindow(): WindowRequestResult` — default `RequestError.Unsupported`; AppKit may return `RequestError.Ignored` when no current `NSEvent` is available (see DEFERRED.md) |
