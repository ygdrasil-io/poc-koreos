...
Le jalon M2 valide l'intégration bout-en-bout de la stack graphique Kadre sur macOS :
...
**Résultat** : un triangle RGB tourne à ~120 fps dans une fenêtre Kadre redimensionnable, sans JNA ni Rococoa — uniquement Panama FFM (JDK 25).
...
### 4. Découplage `hello-triangle` / `kadre-appkit`

Le sample appelle directement `getMetalLayerFromNsView()` via Panama FFM plutôt que de passer par `kadre-appkit`. C'est intentionnel pour maintenir le sample indépendant, mais une API `RawWindowHandle → NativeAddress` dans `kadre-appkit` simplifierait les futurs samples.
