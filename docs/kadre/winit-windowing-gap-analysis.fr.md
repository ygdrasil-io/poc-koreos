...
| `Window.drag_window()` | `Window.dragWindow()` | deferred | Kadre retourne `WindowRequestResult` au lieu d'un no-op `Unit`; Win32 enfile les appels cross-thread vers le thread fenetre comme winit, puis le demarrage natif reste fire-and-forget; les autres backends desktop restent a cabler. |
| `Window.drag_resize_window()` | `Window.dragResizeWindow()` | deferred | Kadre retourne `WindowRequestResult` au lieu d'un no-op `Unit`; Win32 enfile les appels cross-thread vers le thread fenetre comme winit, puis le demarrage natif reste fire-and-forget; les autres backends desktop restent a cabler. |
