...
| `Window.drag_window()` | `Window.dragWindow()` | deferred | Kadre retourne `WindowRequestResult` au lieu d'un no-op `Unit`; AppKit utilise le `NSEvent` courant sur main thread et retourne `RequestError.Ignored` quand aucun evenement courant n'est disponible, mais le marshalling off-main-thread winit reste a cabler; Win32 enfile les appels cross-thread vers le thread fenetre comme winit, puis le demarrage natif reste fire-and-forget; X11/Wayland restent a cabler. |
| `Window.drag_resize_window()` | `Window.dragResizeWindow()` | deferred | Kadre retourne `WindowRequestResult` au lieu d'un no-op `Unit`; Win32 enfile les appels cross-thread vers le thread fenetre comme winit, puis le demarrage natif reste fire-and-forget; AppKit est unsupported comme winit, X11/Wayland restent a cabler. |
| `Window.show_window_menu()` | `Window.showWindowMenu()` | deferred | Kadre retourne `WindowRequestResult` au lieu d'un no-op `Unit`; le menu systeme Win32 est cable, les autres backends desktop restent a cabler. |
...
| Result/error support | `WindowRequestResult`, `SurfaceSizeRequestResult`, `RequestError` | implemented | Types utilises par `requestSurfaceSize`, les requetes curseur fallibles (`setCursorGrab`, `setCursorPosition`, `setCursorHittest`) et les requetes de gestion fenetre (`showWindowMenu`, `dragWindow`, `dragResizeWindow`), avec `RequestError.Ignored` pour les requetes ignorees par la plateforme. |
