...
Kadre **v0.1.0** est livré : 3 plateformes (macOS, iOS, Android), API stable inspirée de winit, intégration validée avec wgpu4k (triangle rendu), artefact publié Maven Central (`org.graphiks.kadre:0.1.0`).
...
2. **v0.2.0** — étendre Kadre à **6 plateformes** (ajout Web, Windows, Linux) et livrer une démo technique **Pong cross-platform** comme proof point.
...
- **Compose-on-Kadre** : POC d'évaluation **après** v0.2.0 (2 sem R&D, voir §11)
...
| Équipe Kadre | Implémentation rémédiation + 3 nouveaux backends + Pong |
...
|--------|-----------|------------------|
| `kadre-core` | Livré | Ajout variants RawWindowHandle (Web/Win32/Xlib/Wayland), inchangé sur fondamentaux |
| `kadre-appkit` | Livré | Inchangé (rémédiation mineure si applicable) |
| `kadre-uikit` | Livré | Inchangé |
| `kadre-android` | Livré | Rémédiation : `AndroidEventLoop.createWindow` ne doit plus throw |
| `kadre` (facade) | Livré | Ajout actuals pour Web (JS+WASM), Windows, Linux |
...
| **`kadre-web-common`** | — | **Nouveau** : abstractions partagées Web (DOM events, lifecycle pagehide/show) |
| **`kadre-js`** | — | **Nouveau** : backend Web Kotlin/JS |
| **`kadre-wasm`** | — | **Nouveau** : backend Web Kotlin/Wasm |
| **`kadre-win32`** | — | **Nouveau** : backend Windows via kextract FFM |
| **`kadre-x11`** | — | **Nouveau** : backend Linux X11 via kextract FFM |
| **`kadre-wayland`** | — | **Nouveau** : backend Linux Wayland via kextract FFM |
...
- Compose-on-Kadre : POC après v0.2.0
...
- Lib v0.2.0 publiée Maven Central avec les **9 modules** (`kadre-core`, `kadre-appkit`, `kadre-uikit`, `kadre-android`, `kadre-js`, `kadre-wasm`, `kadre-win32`, `kadre-x11`, `kadre-wayland`, + facade `kadre`)
...
|-------|--------------------|
| v0.1.1 | Tag créé, mkdocs branded Kadre, samples Android partagés, créateWindow fonctionnel sur Android. |
...
| (Hors v0.2) POC Compose-on-Kadre | 2 sem | T0 + 15 sem (septembre) |
...
| D8 | **Compose-on-Kadre** : POC d'évaluation 2 sem **après** v0.2.0 |
...
- Si Kotlin/JVM perd la cible bytecode JDK 25 avant Kadre v1.0 → fallback JDK 22 (compromise FFM stable + adoption plus large).
...
### Mapping winit → Kadre (ajouts v0.2)
