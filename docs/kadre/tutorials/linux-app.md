# Tutorial: integrate Kadre in a Linux application

This tutorial walks you through creating a native Linux window with Kadre on JVM 25 step by step. You will end up with a working window on X11 or Wayland that responds to mouse and keyboard events and closes cleanly.
...
Kadre uses Vulkan for low-level rendering and X11/Wayland protocols for window management. Install the development headers before building:
...
`ApplicationHandler` is the central interface: Kadre calls it for each lifecycle and window event. The implementation is identical for X11 and Wayland — backend detection is transparent.
...
class MyAppHandler : ApplicationHandler {
    ...
    override fun canCreateSurfaces(eventLoop: ActiveEventLoop) {
        window = eventLoop.createWindow(
            WindowAttributes(
                title = "My Linux application — Kadre",
                resizable = true,
            )
        )
    }
...
The Kadre facade `EventLoop` class automatically detects the operating system via `System.getProperty("os.name")` and delegates to the Linux backend by reflection. Inside the Linux backend, Kadre then chooses between X11 and Wayland based on the environment variables present (see [X11 vs Wayland choice](#x11-vs-wayland-choice) below). No platform-specific import is needed in your code.
...
Wayland exposes the screen scale factor via the `wl_output` protocol (integer since Wayland 1.x) and `wp_fractional_scale_v1` for fractional values (e.g., `1.25×`, `1.5×`). Kadre translates these to `WindowEvent.ScaleFactorChanged(factor)`.

**X11 — Xft.dpi heuristic:**
X11 has no standardized DPI mechanism. Kadre reads the `Xft.dpi` X resource via `XGetDefault(display, "Xft", "dpi")`. This resource is set by GNOME, KDE, and most window managers. If absent, Kadre falls back to the physical DPI computed from `DisplayWidth` / `DisplayWidthMM`, or a default of 96 DPI.
...
Kadre requests server-side decorations first. If the compositor does not support `xdg-decoration` (e.g., Sway without `xwayland`), Kadre falls back automatically to CSD mode with client-drawn decorations.
...
!!! note "No title bar under Sway?"
    ...
    decorations by default. This is expected window manager behavior, not a Kadre bug.
...
CI environments (GitHub Actions, GitLab CI) have no graphics server. Two approaches allow running Kadre tests in *headless* mode:
...
`weston` is the reference Wayland compositor. Its `headless` backend creates a compositor without a physical display, allowing Kadre's native Wayland backend to be tested in CI.
...
| Force a backend | `KADRE_LINUX_BACKEND=wayland` or `=x11` |
