# Tutoriel : intégrer Kadre dans une page web

Ce tutoriel vous guide pas-à-pas pour afficher une surface Kadre dans une page HTML avec Kotlin/JS. Vous obtiendrez un canvas interactif qui répond aux événements pointeur et clavier grâce à la boucle d'événements `requestAnimationFrame`.
...
Créez (ou adaptez) votre fichier `build.gradle.kts` avec la dépendance `kadre-js` et la cible Kotlin/JS configurée pour le navigateur :
...
kotlin {
    ...
    sourceSets {
        jsMain {
            dependencies {
                // Façade Kadre pour le navigateur — Kotlin/JS IR
...
!!! note "Kotlin/JS IR obligatoire"
    Kadre utilise `@JsExport` et les optimisations DCE du compilateur IR.
...
`ApplicationHandler` est l'interface centrale : Kadre l'appelle pour chaque événement du cycle de vie et de la fenêtre. Côté web, la « fenêtre » correspond à un élément `<canvas>` identifié par son id CSS.
...
<head>
    <meta charset="UTF-8">
    <title>Mon application Kadre Web</title>
    <style>
...
!!! warning "Le canvas doit exister avant `runApp`"
    Kadre résout le canvas par son id CSS au moment de l'appel `createWindow`.
...
!!! tip "DPI haute densité — `devicePixelRatio`"
    ...
    Kadre exposera cette valeur via `Window.scaleFactor` dans une version future
...
Pour une performance maximale du rendu, utilisez la cible `kadre-wasm` (Kotlin/Wasm) à la place de `kadre-js`. Kotlin/Wasm compile vers WebAssembly, ce qui offre des performances d'exécution proches du natif pour le code de rendu.
...
!!! tip "Quand choisir Kotlin/Wasm ?"
    ...
    Pour la majorité des cas d'usage, `kadre-js` suffit. Optez pour `kadre-wasm`
