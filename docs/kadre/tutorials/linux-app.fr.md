# Tutoriel : intégrer Kadre dans une application Linux

Ce tutoriel vous guide pas-à-pas pour créer une fenêtre native Linux avec Kadre sur JVM 25. Vous obtiendrez une fenêtre opérationnelle sur X11 ou Wayland qui répond aux événements souris et clavier et se ferme proprement.
...
Kadre utilise Vulkan pour le rendu bas-niveau et les protocoles X11/Wayland pour le gestionnaire de fenêtres. Installez les en-têtes de développement avant de compiler :
...
kotlin {
    ...
    sourceSets {
        jvmMain {
            dependencies {
                // Façade publique Kadre — routage automatique Linux/Windows/macOS
...
`ApplicationHandler` est l'interface centrale : Kadre l'appelle pour chaque événement du cycle de vie et de la fenêtre. L'implémentation est identique entre X11 et Wayland — la détection du backend est transparente.
...
class MyAppHandler : ApplicationHandler {
    ...
    override fun canCreateSurfaces(eventLoop: ActiveEventLoop) {
        window = eventLoop.createWindow(
            WindowAttributes(
                title = "Mon application Linux — Kadre",
                resizable = true,
            )
        )
    }
...
La classe `EventLoop` de la façade Kadre détecte automatiquement le système d'exploitation via `System.getProperty("os.name")` et délègue au backend Linux par réflexion. À l'intérieur du backend Linux, Kadre choisit ensuite entre X11 et Wayland selon les variables d'environnement présentes (voir la section [Choix X11 vs Wayland](#choix-x11-vs-wayland) ci-dessous). Aucun import plateforme-spécifique n'est nécessaire dans votre code.
...
KADRE_LINUX_BACKEND=wayland ./gradlew run
...
Wayland expose le facteur d'échelle de l'écran via le protocole `wl_output` (entier depuis Wayland 1.x) et `wp_fractional_scale_v1` pour les valeurs fractionnaires (ex. `1.25×`, `1.5×`). Kadre traduit ces valeurs en `WindowEvent.ScaleFactorChanged(factor)`.

**X11 — heuristique Xft.dpi :**
X11 n'a pas de mécanisme DPI standardisé. Kadre lit la ressource X `Xft.dpi` via `XGetDefault(display, "Xft", "dpi")`. Cette ressource est définie par GNOME, KDE et la plupart des gestionnaires de fenêtres. Si elle est absente, Kadre se rabat sur le DPI physique calculé depuis `DisplayWidth` / `DisplayWidthMM`, ou sur une valeur par défaut de 96 DPI.
...
Kadre demande les *server-side decorations* en priorité. Si le compositeur ne supporte pas `xdg-decoration` (ex. Sway sans `xwayland`), Kadre repasse automatiquement en mode CSD avec des décorations dessinées côté client.
...
!!! note "Pas de barre de titre sous Sway ?"
    ...
    non un bug Kadre. Utilisez les raccourcis Sway (`$mod+Shift+q`, etc.) pour contrôler
    la fenêtre.
...
Les environnements CI (GitHub Actions, GitLab CI) n'ont pas de serveur graphique. Deux approches permettent de lancer les tests Kadre en mode *headless* :
    ...
    steps:
      ...
      - name: Run tests with Xvfb
        run: |
          ...
          export KADRE_LINUX_BACKEND=x11
          ./gradlew :kadre-core:jvmTest :kadre:jvmTest
```
...
`weston` est le compositeur de référence Wayland. Son backend `headless` crée un compositeur sans affichage physique, ce qui permet de tester le backend Wayland natif de Kadre en CI.
...
| Forcer un backend | `KADRE_LINUX_BACKEND=wayland` ou `=x11` |
