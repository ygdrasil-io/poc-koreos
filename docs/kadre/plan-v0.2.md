...
Kadre **v0.1.0** has shipped: 3 platforms (macOS, iOS, Android), stable API inspired by winit, validated integration with wgpu4k (rendered triangle), artifact published to Maven Central (`org.graphiks.kadre:0.1.0`).
...
2. **v0.2.0** — extend Kadre to **6 platforms** (adding Web, Windows, Linux) and deliver a technical **cross-platform Pong demo** as a proof point.
...
- **Compose-on-Kadre**: evaluation POC **after** v0.2.0 (2-week R&D, see §11)
...
| Kadre team | Remediation + 3 new backends + Pong implementation |
...
|--------|-----------|-------------------|
| `kadre-core` | Delivered | Add RawWindowHandle variants (Web/Win32/Xlib/Wayland), fundamentals unchanged |
| `kadre-appkit` | Delivered | Unchanged (minor remediation if applicable) |
| `kadre-uikit` | Delivered | Unchanged |
| `kadre-android` | Delivered | Remediation: `AndroidEventLoop.createWindow` must no longer throw |
| `kadre` (facade) | Delivered | Add actuals for Web (JS+WASM), Windows, Linux |
...
| **`kadre-web-common`** | — | **New**: shared Web abstractions (DOM events, pagehide/pageshow lifecycle) |
| **`kadre-js`** | — | **New**: Web backend Kotlin/JS |
| **`kadre-wasm`** | — | **New**: Web backend Kotlin/Wasm |
| **`kadre-win32`** | — | **New**: Windows backend via kextract FFM |
| **`kadre-x11`** | — | **New**: Linux X11 backend via kextract FFM |
| **`kadre-wayland`** | — | **New**: Linux Wayland backend via kextract FFM |
...
- Compose-on-Kadre: POC after v0.2.0
...
- Lib v0.2.0 published to Maven Central with all **9 modules** (`kadre-core`, `kadre-appkit`, `kadre-uikit`, `kadre-android`, `kadre-js`, `kadre-wasm`, `kadre-win32`, `kadre-x11`, `kadre-wayland`, + facade `kadre`)
...
|-----------|---------------------|
| v0.1.1 | Tag created, mkdocs branded Kadre, Android samples shared, createWindow functional on Android. |
...
| (Outside v0.2) Compose-on-Kadre POC | 2 weeks | T0 + 15 weeks (September) |
...
| D8 | **Compose-on-Kadre**: 2-week evaluation POC **after** v0.2.0 |
...
- If Kotlin/JVM drops the JDK 25 bytecode target before Kadre v1.0 → fallback JDK 22 (compromise: stable FFM + wider adoption).
...
### winit → Kadre mapping (v0.2 additions)
