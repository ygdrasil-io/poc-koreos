...
- **Cursor** — 25 `CursorIcon` shapes, visibility, grab, warp, hit-testing; custom RGBA cursors (`CursorImage` / `CustomCursor`, backend-dependent)
...
| CursorIcon | real | real | real | partial* | real (CSS) | no-op | no-op |
| CursorGrab Locked | real | real | real | partial* | real* | unsupported | unsupported |
| CursorGrab Confined | unsupported* | real | real | partial* | unsupported | unsupported | unsupported |
...
| Custom cursors | real | real | partial* | partial* | real | no-op | no-op |

`real` = implemented. `real*` / `partial*` / `no-op*` = implemented or documented with platform caveats, see [DEFERRED.md](https://github.com/ygdrasil-io/poc-koreos/blob/master/DEFERRED.md). `TODO` = API defined, backend wiring pending. `—` = not applicable on this platform. Web `CursorGrab Locked` submits a Pointer Lock request; browser grant is asynchronous/user-gesture dependent. Web cursor hit-testing uses CSS `pointer-events`. For Linux, `ModifiersChanged` is wired for key transitions and focus reset/rehydration; XKB locked/latched semantics remain future work.
...
Residual deferred items (API defined, backend wiring pending or platform-limited): rich IME capability/update APIs, DnD/gesture/Occluded backend gaps, Wayland optional protocol/runtime coverage, and native backend gaps. Full list: [DEFERRED.md](https://github.com/ygdrasil-io/poc-koreos/blob/master/DEFERRED.md).
