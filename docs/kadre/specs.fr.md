...
| `Window::set_content_protected()` | `Window.setContentProtected(protected: Boolean): WindowRequestResult` — `RequestError.Unsupported` par défaut; AppKit et Win32 sont câblés localement, tandis que X11 est un success no-op comme winit (cf. DEFERRED.md) |
