...
Kadre est une bibliothèque Kotlin Multiplatform qui abstrait, pour les plateformes Apple Desktop, Apple Mobile et Android :
...
|--------|------------|---------|--------------------|
| `kadre-core` | jvm, android, iosX64/Arm64/SimArm64 | — (Kotlin pur) | non |
| `kadre-appkit` | jvm | FFM JVM 25 via **kextract** | non (linkage runtime AppKit) |
| `kadre-uikit` | iosX64, iosArm64, iosSimulatorArm64 | **cinterop** Kotlin/Native (frameworks Apple built-in) | non |
| `kadre-android` | android | Aucune, Android SDK + `android.view.Surface` exposée brute | non |
| `kadre` (facade) | toutes | `expect`/`actual` | non |

**Conséquence** : 3 toolchains de binding coexistent. Le contrat `kadre-core` les force à converger vers la même API publique.

---

## 3. API publique (`kadre-core`)
...
**Aucune lib native côté Kadre** — c'est intentionnel ([Strategy A](./plan.md#11-décisions-darchitecture-déjà-actées)).
...
- Interception des events :
  - **Subclass** `KadreApplication : NSApplication` qui override `sendEvent:`.
  - Subclass `KadreAppDelegate : NSObject<NSApplicationDelegate>` pour `applicationDidFinishLaunching:` etc.
  - Subclass `KadreWindowDelegate : NSObject<NSWindowDelegate>` pour `windowDidResize:`, `windowShouldClose:`, etc.
...
- Entry point : `KadreActivity : AppCompatActivity` héberge un `SurfaceView` plein écran.
