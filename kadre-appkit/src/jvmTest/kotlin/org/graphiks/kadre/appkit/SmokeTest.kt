...
/**
 * Smoke test — verifies that the Kotlin/FFM bindings provided by kffi-objc
 * compile correctly and that the published AppKit types are accessible.
 *
 ...
 * published companions/interfaces.
 */
class SmokeTest {

    @Test
    fun `the published AppKit classes are accessible`() {
    ...
    fun `the AppKit delegate interfaces are importable`() {
        ...
        // are published as accessible Kotlin interfaces
    ...
    fun `ObjCRuntime is accessible`() {
        // Verify that the published FFM/ObjC runtime bridge compiles
