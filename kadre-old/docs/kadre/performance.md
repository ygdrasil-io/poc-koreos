...
`org.graphiks.kadre.core.FrameTimingTracer` measures the
...
On JVM, activation can be gated on `-Dkadre.tracing=true` at backend startup (reading the system property → `FrameTimingTracer.enabled = true`).
