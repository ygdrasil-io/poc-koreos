...
namespace `org.graphiks.kadre` (vérification DNS ou SCM requise).
...
```bash
# 1a. Changer version=1.0.0-SNAPSHOT → version=1.0.0 dans gradle.properties
...
# Attendu : kadre-core-1.0.0.jar
#           kadre-core-1.0.0-sources.jar
#           kadre-core-1.0.0-javadoc.jar
#           kadre-core-1.0.0.pom          (signé : .asc)
#           kadre-core-1.0.0.module       (signé : .asc)
```
...
|------------------|-------------------|---------------------------------------|
| `kadre-core`    | `kadre-core`     | Interfaces KMP pures (commonMain)     |
| `kadre`         | `kadre`          | Façade publique KMP (jvm + iOS + android) |
| `kadre-appkit`  | `kadre-appkit`   | Backend macOS (AppKit, JVM 25, FFM)   |
| `kadre-uikit`   | `kadre-uikit`    | Backend iOS (UIKit, Kotlin/Native)    |
| `kadre-android` | `kadre-android`  | Backend Android (SurfaceView)         |
...
### kadre-android : publications iOS désactivées

`kadre-android` utilise le plugin `kmp-library` qui ajoute des cibles iOS, mais
ce module ne contient que du code Android. Les publications iOS/JVM sont
intentionnellement désactivées via `afterEvaluate` dans `kadre-android/build.gradle.kts`.
