...
const [actualPath, baselinePath, diffOut, toleranceArg, summaryOut] = process.argv.slice(2);
...
if (summaryOut) {
  ...
  fs.writeFileSync(summaryOut,
    `### Régression visuelle — hello-triangle (macOS, readback GPU)\n\n${line}\n\n` +
    `_Captures (baseline / courante / diff) dans les artefacts du run._\n`);
}
