...
| KFFI-OBJC-002 | open | high | AppKit scalars | Corriger les signatures générées qui exposent `NSApplicationActivationPolicy`, `NSEventType` et `NSEventModifierFlags` comme `MemorySegment` au lieu de scalaires/enums NSInteger/NSUInteger ; conserver `NSPoint` comme struct typée | activation policy standalone et événement synthétique de réveil AppKit sans `ObjCRuntime.msgSend` générique | appel générique KFFI documenté et borné seulement si nécessaire | issue KFFI à créer |
