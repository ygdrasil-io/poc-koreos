...
| `WebWindowProvider.open` (callback host) | `Success(WebWindowHost)` ou failure transformée en `WindowRequestOutcome.Rejected` | exactement le set de `WindowRequestOutcome.Rejected` | synchrone ; aucun owner ni autorité d’interaction n’est transféré avant validation du host retourné. Le callback ne crée pas de browsing context : il fournit un élément déjà créé dans un contexte distinct, dont Kadre ouvre seulement la nouvelle session. Une exception devient `PlatformFailure(Web, "WebWindowProvider", "callback-exception")`; une failure hors set devient le même domain avec code `"invalid-failure"`. |
...
`WindowRequestOutcome.Rejected.failure` admet exactement `Unsupported(RequestWindow)`, `Invalid(field)`, `Interaction(reason)`, `Busy(Host)`, `Closed(Host)`, `ParentScopeCancelled`, `Limit(Window)`, `Temporary` et `Platform`. `Busy(Host)` est admis uniquement lorsqu’un `WebWindowProvider` retourne un élément déjà possédé par une session vivante. `ParentScopeCancelled` est admis uniquement lors de la validation du nouveau host retourné par `WebWindowProvider`; il ne décrit pas la scope du requester. `RequesterDetached` et `Cancelled` restent des outcomes dédiés et ne sont pas dupliqués comme failures ; `UserCancelled(RequestWindow)` n’est donc pas admis ici.
