...
/**
 * Extraction par variante. Elle vit côté gateway parce qu'un `InputEvent` n'est pas
...
internal fun describeInputEvent(event: InputEvent): InputEventSummary = when (event) {
    is InputEvent.Key -> InputEventSummary(
        kind = "Touche",
        detail = (event.logicalKey as? LogicalKey.Character)?.value
            ?: (event.logicalKey as? LogicalKey.Named)?.value?.name
            ?: "touche non identifiée",
    )
    ...
    is InputEvent.PointerButtonChanged -> InputEventSummary("Bouton", "${event.button} (${event.buttonState.name})")
    is InputEvent.Scrolled -> InputEventSummary("Défilement", null)
    is InputEvent.TouchChanged -> InputEventSummary("Tactile", event.phase.name)
    is InputEvent.Gesture -> InputEventSummary("Geste", event.kind.name)
    ...
    is InputEvent.StateReset -> InputEventSummary("État d'entrée réinitialisé", event.reason.name)
}

private fun coordinates(position: LogicalPoint): String = "${position.x} × ${position.y}"
