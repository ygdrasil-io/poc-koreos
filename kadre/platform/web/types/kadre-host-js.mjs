 ...
 * from a sibling module of this package, so a Kotlin/JS application that links this library and
 * publishes its own bindings is the instance the shim drives. A Kotlin/JS module publishes its
 * exported names at load time on the module object, which is the registry itself for a library build
 * and its package path for an application bundle; `publishHostBindings()` also writes the eight flat
 * names there. Nothing in this module loads the Kotlin library on its own.
 *
