 ...
 * application must publish its bindings before it uses this package: it calls
 * `publishHostBindings()` — the Kotlin half of section 6, which also produces the opaque factory key
 * this shim carries back to `KadreWeb.attach` — and `attach` resolves them from the shared registry
 * at call time. A Kotlin/JS library build publishes its exported names under its package path on that
 * same registry object, which is the fallback a Kotlin/JS embedder that never calls the publisher
 * gets. A page that publishes nothing is reported with the explicit error below.
 *
