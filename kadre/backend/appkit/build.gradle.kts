...
if (System.getProperty("os.name", "").let { name ->
    ...
    tasks.register<JavaExec>("phase4InputHarness") {
        group = "verification"
        description = "Runs the external interactive AppKit Phase 4 input harness."
        dependsOn(tasks.named("jvmTestClasses"))
        classpath = jvmTest.get().classpath
        mainClass.set("org.graphiks.kadre.internal.appkit.manual.Phase4InputHarnessKt")
        workingDir(rootProject.projectDir)
        jvmArgs(
            "-XstartOnFirstThread",
            "--enable-native-access=ALL-UNNAMED",
    ...
    tasks.register<JavaExec>("phase5AdvancedWindowHarness") {
        group = "verification"
        description = "Runs the external interactive AppKit Phase 5 advanced-window harness."
        dependsOn(tasks.named("jvmTestClasses"))
        classpath = jvmTest.get().classpath
        mainClass.set("org.graphiks.kadre.internal.appkit.manual.Phase5AdvancedWindowHarnessKt")
        workingDir(rootProject.projectDir)
        jvmArgs(
            "-XstartOnFirstThread",
            "--enable-native-access=ALL-UNNAMED",
    ...
    tasks.register<JavaExec>("phase7DropHarness") {
        group = "verification"
        description = "Runs the external interactive AppKit Phase 7 drag-and-drop harness."
        dependsOn(tasks.named("jvmTestClasses"))
        classpath = jvmTest.get().classpath
        mainClass.set("org.graphiks.kadre.internal.appkit.manual.Phase7DropHarnessKt")
        workingDir(rootProject.projectDir)
        jvmArgs(
            "-XstartOnFirstThread",
            "--enable-native-access=ALL-UNNAMED",
    ...
    tasks.register<JavaExec>("phase8RawInputHarness") {
        group = "verification"
        description = "Runs the external interactive AppKit Phase 8 raw-input harness."
        dependsOn(tasks.named("jvmTestClasses"))
        classpath = jvmTest.get().classpath
