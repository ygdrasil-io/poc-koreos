...
| KFFI-OBJC-008 | closed | blocking | CoreGraphics listen-only event tap | API KFFI `CGListenOnlyEventTap` construite au-dessus des symboles générés `CGEventTapCreate`, `CGEventTapEnable`, `CGEventGetIntegerValueField`, `CGPreflightListenEventAccess` et `CGRequestListenEventAccess`; le callback reste listen-only et retourne l’événement natif sans mutation | Phase 8 : source raw input AppKit globale, permission Input Monitoring, réactivation après timeout et copie immédiate des deltas | aucun downcall, callback FFM ou pointeur dans Kadre ; Kadre ne consomme que l’adapter KFFI typé publié | [KFFI #64](https://github.com/Graphiks-org/kffi/pull/64), artifact `org.graphiks:kffi-objc:1.0.0-SNAPSHOT` |
