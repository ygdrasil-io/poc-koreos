...
| KFFI-OBJC-005 | open | blocking | synthèse et injection d’événements AppKit | fournir une voie publique KFFI typée, owner-safe et testable pour injecter un scroll AppKit contenant delta discret/précis, phase et momentum dans la file `NSApplication`; le choix `NSEvent` ou `CGEvent` appartient à KFFI | preuve O3 Phase 4 du scroll réel par la file AppKit, sans appel direct d’un mapper Kadre | aucun `MemorySegment` brut, factory Objective-C générique, block ou downcall local dans Kadre | `APPKIT-PHASE-4-INPUT-DESIGN.md`, scénario O3 `appkit-input-scroll` |
