...
- [x] Écrire les tests O2 rouges : canonisation, validation borderless,
  mutation combinée, cancellation, révision et event ordering.
- [x] Étendre le set de routage et le candidat `WindowUpdate` à
  `decorations`/`systemButtons`, tout en gardant les capabilities publiques
  inchangées.
- [x] Étendre le snapshot effectif et `PropertiesChanged` aux deux champs.
- [x] Vérifier `:kadre:runtime:jvmTest` et les mappings O2 réservés.

## PR 3 — Port AppKit privé

- [x] Écrire les tests O3 rouges pour le style mask, les boutons masqués,
  l'annulation avant setter et l'isolation de deux peers.
- [x] Étendre la cible/snapshot privée et la barrière de commit du peer.
- [x] Appliquer et relire style mask et boutons avec les bindings KFFI
  générés ; préserver tous les bits non possédés.
- [x] Vérifier les tests déterministes et macOS réels de l'adapter AppKit.

## PR 4 — Activation publique et evidence

- [x] Écrire le test rouge de capability publique, de mutation combinée et de
  state-before-event sur une vraie fenêtre macOS.
- [x] Ajouter les deux capabilities publiques AppKit, sans modifier les autres
  champs hors scope.
- [x] Activer `WIN-003` et `APK-008`, ajouter leurs mappings et les gates.
- [x] Ajouter un harness manuel non bloquant qui enregistre les quatre états
  visuels définis par le contrat.
- [x] Rejouer la suite AppKit, le validateur et `test-kadre-appkit-contracts.sh`.
