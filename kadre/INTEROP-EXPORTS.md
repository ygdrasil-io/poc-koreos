...
`KadreWeb.attach` possède un `MainScope` interne par session qu’il attache et le cancel après son échec d’attach ou sa terminaison. Cela n’introduit aucun global : le handle retourné possède la scope de sa session initiale ; une session créée par `KadreWebWindowProvider` possède sa propre scope, même si `OpenedInNewSession` n’expose pas son handle au requester. La fermeture du requester n’annule jamais une session déjà ouverte dans ce nouveau browsing context. Un échec d’attach lève une `KadreHostError` dont `failure: KadreFailure`; les opérations asynchrones promise rejetteraient uniquement par cancellation/erreur de programmation, tandis qu’un outcome fonctionnel reste une value résolue.
