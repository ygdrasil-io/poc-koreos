...
- Les opérations Web qui exigent la transient user activation passent par `InteractionContext`. Kadre ne crée jamais de browsing context sur Web : popup, iframe et placement DOM restent host-owned. Les requêtes suspendues ordinaires retournent `InteractionRequired` lorsqu’elles arrivent trop tard.
