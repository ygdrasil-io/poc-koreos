...
- Kadre ne crée jamais de `<canvas>`, `<div>` ou popup et ne choisit aucun emplacement dans le DOM. Sans `WebWindowProvider`, `requestWindow` retourne `Success(WindowRequest)` déjà terminal en `Rejected(Unsupported(RequestWindow))`. Un provider peut demander un nouveau browsing context ; son élément doit avoir un `ownerDocument.defaultView` non nul et distinct du browsing context origine, sinon la requête atteint `Rejected(InvalidRequest("element.ownerDocument"))`. Un host valide devient une nouvelle session et produit `OpenedInNewSession`, jamais `OpenedHere`.
