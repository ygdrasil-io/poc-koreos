...
- `StopWhenDetached` vérifie `isConnected` et le `ownerDocument` initial à la livraison du batch `MutationObserver`. L’observer est installé sur le `Document` ou le `ShadowRoot` contenant l’élément et il est réinstallé après chaque batch qui change ce root. Un reparenting terminé avant cette livraison ne ferme la session que dans le même document ; un transfert vers un autre document la termine, même si l’élément est reconnecté. Un élément encore détaché la termine et sa réinsertion exige une nouvelle session.
