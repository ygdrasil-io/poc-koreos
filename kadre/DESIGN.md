...
- `StopWhenDetached` vérifie `isConnected` et le `ownerDocument` initial à la livraison du batch `MutationObserver`. L’observer est installé sur le `Document` ou le `ShadowRoot` contenant l’élément et il est réinstallé après chaque batch qui change ce root. Un reparenting terminé avant cette livraison conserve la session seulement si l’élément reste dans son document initial ; tout transfert inter-document la termine, même si l’élément est reconnecté. Un élément encore détaché la termine et sa réinsertion exige une nouvelle session.
- `Manual` ignore le détachement DOM, y compris un detach/reinsert, et exige un `requestStop` explicite ; tant que son élément reste déconnecté, son lifecycle demeure `Background + Inactive`. Dès l’attach initialement déconnecté, ou après livraison d’un batch où il le reste, Kadre surveille sa reconnexion par `requestAnimationFrame` sur le browsing context d’origine jusqu’à reconnexion ou terminaison, puis réinstalle l’observer `Document`/`ShadowRoot`. Une reconnexion tardive dans un autre `ShadowRoot` du même document est ainsi observée ; un autre document termine la session.
- `visibilitychange` pilote `Foreground` seulement pour un document visible et un élément connecté. `focus`/`blur` du browsing context et `focusin`/`focusout` de l’élément ou de son subtree pilotent `Active`/`Inactive` : une surface n’est active que si elle est foreground et reçoit l’input. Un document background, une surface déconnectée ou un autre host ayant le focus publient toujours `Inactive`.
- Aucun détournement du titre comme ID DOM.
- Sessions multiples possibles sur une même page, mais un `HTMLElement` ne peut être attaché que par une seule session vivante. Un second `attachKadre` sur le même élément retourne `Busy(Host)` sans installer de listener, sans modifier la session existante et sans transférer ses capabilities ; l’élément redevient admissible seulement après la terminaison de son owner.
- Kadre ne crée jamais de `<canvas>`, `<div>` ou popup et ne choisit aucun emplacement dans le DOM. Sans `WebWindowProvider`, `requestWindow` retourne `Success(WindowRequest)` déjà terminal en `Rejected(Unsupported(RequestWindow))`. Le provider fournit uniquement un élément déjà créé dans un browsing context **distinct** : son `ownerDocument.defaultView` doit être non nul et distinct du browsing context origine, sinon la requête atteint `Rejected(InvalidRequest("element.ownerDocument"))`. Kadre crée alors une nouvelle session sur ce host et produit `OpenedInNewSession`, jamais `OpenedHere`; ce nom décrit la nouvelle session Kadre, pas la création d’un contexte natif. Le provider ne crée ni popup, ni iframe, ni browsing context.
- Les opérations Web qui exigent la transient user activation passent par `InteractionContext`. Kadre ne crée jamais de browsing context sur Web : popup, iframe et placement DOM restent host-owned. `InteractionAction.OpenWindow` reste `Unsupported` sur Web même si l’action demeure dans l’API commune pour d’autres adapters. Les requêtes suspendues ordinaires retournent `InteractionRequired` lorsqu’elles arrivent trop tard.
- Même contrat public en JS et Wasm, avec le type SDK exact `org.w3c.dom.HTMLElement` fourni par chaque toolchain : les déclarations utilisant `HTMLElement` sont distinctes en `jsMain` et `wasmJsMain`; seul leur noyau interne, sans type DOM public, est partagé.
