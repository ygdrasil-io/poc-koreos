...
# Runs one command in its own process group, kills that whole group on timeout,
# and preserves the terminal status observed by the caller.
run_with_timeout() {
    local timeout_seconds="$1"
    shift
    /usr/bin/perl -MPOSIX=setpgid -MErrno=EINTR,ESRCH -e '
        use strict;
        use warnings;
        use Time::HiRes ();
        ...
        my $status = $?;
        alarm 0;
        extinguish_group($pid) if defined($terminal_status) && $group_ready;
        exit(
