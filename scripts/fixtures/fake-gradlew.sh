...
if [[ " $* " == *" :kadre:contracts:validator:generateAppKitContractEvidence "* ]]; then
    ...
    for contract_id in APK-001 APK-002 APK-003 APK-004 APK-005 APK-006 APK-007 APK-008 APK-009 APK-010 APK-011 APK-012 APK-013 APK-014 APK-017 APK-018; do
...
if [[ " $* " == *" :kadre:contracts:validator:generateRuntimeContractEvidence "* ]]; then
    ...
    for contract_id in INP-001 INP-002 WIN-001 WIN-002 WIN-003 WIN-004 WIN-005 WIN-006 INT-001 DSP-001 RUN-007 RUN-008; do
        if [[ "${KADRE_FAKE_MISSING_EVIDENCE:-}" != "$contract_id" ]]; then
