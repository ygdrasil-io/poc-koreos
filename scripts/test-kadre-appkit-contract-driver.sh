...
EVIDENCE_FILES=("APK-001.json" "APK-002.json" "APK-003.json" "APK-004.json" "APK-005.json" "APK-006.json" "APK-007.json")
...
rm -rf "$EVIDENCE_DIRECTORY"
TRACE="$TEMP_DIR/missing-apk007.trace"
capture_status observed_status env \
    KADRE_GRADLEW="$FAKE_GRADLE" \
    KADRE_FAKE_GRADLE_TRACE="$TRACE" \
    KADRE_FAKE_EVIDENCE_DIRECTORY="$EVIDENCE_DIRECTORY" \
    KADRE_FAKE_MISSING_EVIDENCE=APK-007 \
    GITHUB_SHA="0123456789abcdef" \
    bash "$DRIVER"

[[ "$observed_status" != "0" ]] || fail "missing APK-007 evidence passed the AppKit contract gate"
[[ "$(wc -l < "$TRACE" | tr -d ' ')" == "2" ]] || fail "missing APK-007 evidence did not run both Gradle phases"
