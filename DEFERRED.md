...
- `WindowButtons` — `setEnabledButtons` / `enabledButtons`: API commune présente; AppKit câblé localement via `NSWindowStyleMaskClosable` / `NSWindowStyleMaskMiniaturizable` et `standardWindowButton(NSWindowZoomButton).setEnabled`; Win32 câblé localement via `WS_MINIMIZEBOX`, `WS_MAXIMIZEBOX` et `EnableMenuItem(SC_CLOSE)`; X11 et Wayland acceptent `setEnabledButtons` en no-op et retournent `WindowButtons.ALL` comme winit; Web/mobile restent `all()`/unsupported documenté à compléter selon les limites natives.
...
| **Wayland** | **curseur** → `libwayland-cursor` (no-op) ; **grab** → `CursorGrabMode.None` success no-op comme winit, `Confined`/`Locked` restent à câbler via pointer-constraints ; **attention** → `xdg_activation_v1` ; **icone fenêtre** → `xdg_toplevel_icon_manager_v1` ; **blur** → `ext_background_effect` / KWin blur ; **texte clavier** → `xkb_state_key_get_utf8` (`text = null`) ; changement dynamique de décoration SSD/CSD ; plein écran exclusif N/A |
