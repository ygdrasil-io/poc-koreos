...
- **Divers fenêtre** : `requestUserAttention` et `memoryWarning` restent no-op par défaut; `setContentProtected` est partiel selon backend et n'est plus un no-op global. `showWindowMenu`, `dragWindow` et `dragResizeWindow` retournent désormais `WindowRequestResult.Failure(RequestError.Unsupported(...))` par défaut. AppKit câble `dragWindow` via le `NSEvent` courant sur le main thread et retourne `RequestError.Ignored` quand aucun événement courant n'est disponible; le marshalling AppKit depuis un thread non-main reste à câbler. Win32 câble le menu système et enfile les drags move/resize cross-thread vers le thread fenêtre comme winit; le démarrage natif reste fire-and-forget. `dragResizeWindow` AppKit reste unsupported comme winit; X11 et Wayland restent à câbler.
